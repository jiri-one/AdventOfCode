...
# input files
main_input = Path(__file__).parent / "input.txt"  # result of this file is PWPWHGFZS
test_input = Path(__file__).parent / "test_input.txt"  # result of this file is MCD
...
for line_index, line in enumerate(raw_lines):
    if "[" not in line and "move" not in line:
        ...
        ]  # split line to list and remove empty elements
        try:
...
for cmd in reversed(move):
    moved_crates = stacks[cmd["src_stack"]][-cmd["crates_count"] :]
    stacks[cmd["src_stack"]] = stacks[cmd["src_stack"]][: -cmd["crates_count"]]
    stacks[cmd["dst_stack"]] = stacks[cmd["dst_stack"]] + moved_crates
