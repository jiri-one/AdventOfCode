...
for line in data:
    (
        ...
        geo_cost_ore,
        geo_cost_obs,
    ) = (int(n) for n in LINE_RE.match(line).groups())
...
class Node:
    ...
    def optimistic_estimate(self):
        ...
        return self.score + self.robots[-1] * mr + (mr + 1) * mr // 2
    ...
    def generate_next_nodes(self):
        ...
        built_a_robot = False
        for robot_i, (cost, current_robot_count, max_cost) in enumerate(
            zip(
            ...
            # for
            if any(c > 0 and r == 0 for c, r in zip(cost, self.robots)):
                continue
            ...
            while minutes_remaining:
                ...
                if all(m >= c for m, c in zip(materials, cost)):
                    # Yes!
                    break
                # Spend this turn gathering resources
                minutes_remaining -= 1
                materials = tuple(m + r for m, r in zip(materials, self.robots))
            if minutes_remaining:
                # Spend resources for robot
                materials = tuple(m - c for m, c in zip(materials, cost))
                minutes_remaining -= 1
                # Robots gather resources
                materials = tuple(m + r for m, r in zip(materials, self.robots))
                robots = tuple(
                    r + (1 if robot_i == i else 0) for i, r in enumerate(self.robots)
                )
                yield replace(
                    self,
                    minutes_remaining=minutes_remaining,
                    materials=tuple(
                        min(m, mc + (mc - r) * minutes_remaining)
                        for m, mc, r in zip(
                            materials,
                            self.blueprint.max_costs,
                            robots,
