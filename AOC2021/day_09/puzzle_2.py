...
def detect_its_basin(row_nr: int, elm_nr: int):
    basins_coordinates = {
        (row_nr, elm_nr)
    }  # it is set, so any element of set is unique
    while True:  # repeat until break is called
        last_basin_len = len(
            basins_coordinates
        )  # We need to check if new elements have added in main for cycle or it is done
        for coordinate in set(
            basins_coordinates
        ):  # main iteration of set (it is actual copy of set, bacause original set we will modify)
            row_index, elm_index = coordinate
            for try_row_i, try_elm_i in [
                (row_index + 1, elm_index),
                (row_index - 1, elm_index),
                (row_index, elm_index + 1),
                (row_index, elm_index - 1),
            ]:  # iteration of "cross adjacent locations"
            ...
            if (
                len(basins_coordinates) == last_basin_len
            ):  # check if any new element is in set, and if not ...
