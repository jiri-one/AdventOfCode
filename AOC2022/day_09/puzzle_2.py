...
class SnakePiece:
    ...
    def move_tail(self):
        if self.cur_head not in self.get_surround_coor_set():
            xt, yt, xh, yh = self.cur_tail[0], self.cur_tail[1], self.cur_head[0], self.cur_head[1]
            if yt == yh and xt != xh:
                if xh > xt: self.cur_tail = (xt + 1, yt) # move tail right
                else: self.cur_tail = (xt-1, yt) # move tail left
