...
class SignalDecoder:
    def __init__(self, signals):
        ...
        self.segments[1] = [char for char in self.seven if not char in self.one][
            0
        ]  # we know, one and seven, so the first segnem will be letter which is not in number one
        ...
        self.segments[6] = [char for char in self.one if char != self.segments[3]][
            0
        ]  # we know chars in number one and we know segment three, so the difference will be segment six
        self.rest_signals()
    ...
    def segment_seven(self):
        ...
        for signal in self.signals:
            if len(signal) == 6:
                result = [
                    char for char in signal if char not in self.four + self.segments[1]
                ]
        ...
        for signal in self.signals:
            if len(signal) == 5:
                result = [
                    char
                    for char in signal
                    if char not in self.four + self.segments[1] + self.segments[7]
                ]
                if result and len(result) == 1:
                    return result[0]
        ...
        for signal in self.signals:
            if len(signal) == 6:
                result = [
                    char
                    for char in signal
                    if char not in self.seven + self.segments[5] + self.segments[7]
                ]
                if result and len(result) == 1:
                    return result[0]
        ...
        for signal in self.signals:
            if len(signal) == 5:
                result = [
                    char
                    for char in signal
                    if char not in self.one + self.segments[1] + self.segments[7]
                ]
                if result and len(result) == 1:
                    return result[0]
