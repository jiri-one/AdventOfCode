...
# helper functions
def mark_surrounding_to_check(x, y, lowest_risk):
    """Mark surrondings elemnts to have to be checked, except the one, which was the entry"""
    print(lowest_risk[0], x, y)
    if lowest_risk[0] != "left":
        if x != 0:
            if arr_way.T[x - 1][y] > arr_way.T[x][y] - arr_risk.T[x][y]:
    ...
    if lowest_risk[0] != "up":
        if y != 0:
            if arr_way.T[x][y - 1] > arr_way.T[x][y] - arr_risk.T[x][y]:
...
while True in arr_check:
    with np.nditer( [arr_risk, arr_check, arr_way],
                flags=['multi_index'],
        ...
        for risk, check, way in ar: # risk is from arr_risk, check is from... etc
            ...
            if check == True:
                ...
                if y <= arr_way.T.shape[1] - 2 and arr_way.T[x][y + 1] != 0:
                    ...
                    down_risk = None
                risk_list = [("left", left_risk), ("right", right_risk), ("up", up_risk), ("down", down_risk)]
                ...
                if len(surrounding_risks) > 0:
                    ...
                    if way[...] == 0 or lowest_risk[1] < way[...]:
                        way[...] = lowest_risk[1]
                        check[...] = False
                        mark_surrounding_to_check(x, y, lowest_risk)
        print(arr_check)
        print(arr_way)
